<script lang="ts">
  ...
  import { PLYMerger } from '../ply-merger.js';
  import {
  	chunksInfo,
  	clearLogs,
  	sessionId,
  ...
  // Merge and download combined PLY
  async function mergeAndDownload() {
    if ($splatChunks.length === 0) {
      toast.error('No chunks available to merge');
      return;
    }

    try {
      const mergedBlob = await PLYMerger.mergeChunks($splatChunks);
      PLYMerger.downloadBlob(mergedBlob, 'combined_gaussian.ply');
      toast.success('Combined PLY file downloaded successfully');
    } catch (error) {
      toast.error(`Failed to merge chunks: ${error}`);
    }
  }
...
<div class="status-panel">
  ...
  <div class="section">
    <h3 class="section-title">
      ...
      {#if $splatChunks.length > 0}
        <button class="btn btn-sm btn-primary" on:click={mergeAndDownload}>
          Download Combined PLY
        </button>
      {/if}
    </h3>
  ...
  <div class="section logs-section">
    <div class="logs-header">
      ...
      <div class="logs-controls">
        ...
        <button class="btn btn-sm" on:click={clearLogs}>
          Clear
        </button>
      </div>
    </div>

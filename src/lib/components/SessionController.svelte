<script lang="ts">
	...
	async function createSession() {
		...
		try {
			...
			if (!connected) {
				addLog(
					'⚠️ WebSocket connection failed, but processing can continue without real-time updates'
				);
	...
	function updateConcurrentUploads(event: Event) {
		const target = event.target as HTMLInputElement;
		const value = parseInt(target.value);
		if (!isNaN(value) && value >= 1 && value <= 5) {
			sessionConfig.update((config) => ({ ...config, concurrentUploads: value }));
		}
	}
		...
		<div class="config-grid">
			...
			<label class="label">
				...
				<input
					type="number"
					class="input"
					value={$sessionConfig.concurrentUploads}
					on:input={updateConcurrentUploads}
					min="1"
					max="5"
					disabled={!!$sessionId}
				/>
	...
	<div class="controls">
		...
		{:else}
			<div class="session-info">
				...
				<button class="btn btn-danger" on:click={closeSession} disabled={isClosingSession}>
					{#if isClosingSession}
						Closing Session...
					{:else}
						Close Session

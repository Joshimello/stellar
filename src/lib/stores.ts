...
export const isDownloadingChunks = writable<boolean>(false);

// Add logging when download state changes
isDownloadingChunks.subscribe((downloading) => {
	if (typeof window !== 'undefined') {
		const timestamp = new Date().toLocaleTimeString();
		console.log(
			`[${timestamp}] 📡 Download state changed: ${downloading ? 'TRUE (uploads paused)' : 'FALSE (uploads resumed)'}`
		);
	}
});
...
export const chunksInfo = writable<
	Array<{
		chunkId: string;
		startFrame: number;
		endFrame: number;
...
export const sessionConfig = writable({
	...
	overlap: 3,
	loopEnable: true,
	uploadDelay: 0, // milliseconds (0 = no delay for speed)
	concurrentUploads: 3 // number of simultaneous uploads
});
